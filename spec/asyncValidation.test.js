...
describe("asyncValidation", () => {
  describe("satisfies", () => {
    it("populates errors based on a custom rule", () => {
      ...
      return validator({ name: "invalid" }, v =>
        v(v.validate("name").satisfies(rule))
      ).then(errors => {
    ...
    it("accepts multiple rules", () => {
      ...
      return validator({ name: "invalid" }, v =>
        v(v.validate("name").satisfies(rule1, rule2))
      ).then(errors => {
    ...
    describe("when a child object does not exist", () => {
      ...
      it("populates child errors", () => {
        return validator(values, v =>
          v(
            v.validateChild({ field: "user" }, user =>
              user(user.validate("name").satisfies(required))
            )
          )
        ).then(errors => {
  ...
  describe("validateChildren", () => {
    describe("when the child array does not exist", () => {
      ...
      it("does not populate errors", () => {
        return validator(values, v =>
          v(
            v.validateChildren({ field: "contacts" }, contacts =>
              contacts(contacts.validate("name").satisfies(required))
            )
          )
        ).then(errors => {
    ...
    describe("when a child array has no errors", () => {
      ...
      it("does not populate errors for the child array", () => {
        return validator(values, v =>
          v(
            v.validateChildren("contacts", contacts =>
              contacts(contacts.validate("name").satisfies(required))
            )
          )
        ).then(errors => {
    ...
    describe("when a child array item has errors", () => {
      ...
      it("populates the errors object correctly", () => {
        return validator(values, v =>
          v(
            v.validateChildren({ field: "contacts" }, contact =>
              contact(contact.validate("name").satisfies(required))
            )
          )
        ).then(errors => {
